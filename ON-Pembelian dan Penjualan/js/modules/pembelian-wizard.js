(function () {
  const customers = [
    { id: "cust-1", name: "PT Bahan Kopi Nusantara", phone: "022-6677-9090", email: "order@bahankopi.id", address: "Jl. Soekarno Hatta No. 91, Bandung", bankName: "Bank BCA", accountHolder: "PT Bahan Kopi Nusantara", accountNumber: "1280098811" },
    { id: "cust-2", name: "CV Sumber Rezeki", phone: "031-7788-1200", email: "admin@sumberrezeki.id", address: "Jl. Rungkut Industri No. 12, Surabaya", bankName: "Bank Mandiri", accountHolder: "CV Sumber Rezeki", accountNumber: "140001882200" },
    { id: "cust-3", name: "PT Kemasan Prima", phone: "021-8899-7722", email: "support@kemasanprima.id", address: "Kawasan Industri Cikarang Blok E2", bankName: "Bank BRI", accountHolder: "PT Kemasan Prima", accountNumber: "002101778811" },
    { id: "cust-4", name: "UD Sinar Jaya", phone: "0274-778-334", email: "sales@sinarjaya.id", address: "Jl. Magelang KM 7, Yogyakarta", bankName: "", accountHolder: "", accountNumber: "" },
    { id: "cust-5", name: "Koperasi Tani Flores", phone: "0380-221-908", email: "kopi@taniflores.id", address: "Jl. Ende Raya No. 17, Flores", bankName: "Bank NTT", accountHolder: "Koperasi Tani Flores", accountNumber: "020100887200" }
  ];

  const catalog = [
    {
      id: "prod-green-bean", type: "Produk", name: "Green Bean Kopi", sellable: true,
      variants: [
        { id: "var-gayo", name: "Arabika Gayo Grade A", unit: "Kg", price: 120000, barcode: "899710100001" },
        { id: "var-flores", name: "Arabika Flores Grade A", unit: "Kg", price: 115000, barcode: "899710100002" },
        { id: "var-robusta", name: "Robusta Temanggung", unit: "Kg", price: 85000, barcode: "899710100003" }
      ]
    },
    {
      id: "prod-milk", type: "Produk", name: "Susu Barista", sellable: true,
      variants: [
        { id: "var-full-cream", name: "Full Cream 1 Liter", unit: "Dus", price: 180000, barcode: "899710200001" },
        { id: "var-oat", name: "Oat Milk 1 Liter", unit: "Dus", price: 310000, barcode: "899710200002" }
      ]
    },
    { id: "prod-cup", type: "Produk", name: "Cup Paper 12 oz", sellable: true, unit: "Karton", price: 2100000, barcode: "899710300001", variants: [] },
    { id: "prod-sugar", type: "Produk", name: "Gula Aren Cair", sellable: true, unit: "Dus", price: 110000, barcode: "899710300002", variants: [] },
    { id: "svc-machine", type: "Jasa", name: "Servis Mesin Kopi", sellable: true, unit: "Kunjungan", price: 650000, variants: [] },
    { id: "svc-logistics", type: "Jasa", name: "Jasa Pengiriman Bahan Baku", sellable: true, unit: "Pengiriman", price: 350000, variants: [] },
    { id: "svc-calibration", type: "Jasa", name: "Kalibrasi Grinder", sellable: true, unit: "Unit", price: 275000, variants: [] }
  ];

  const steps = ["Informasi Nota", "Vendor", "Produk atau Jasa", "Pembayaran", "Preview"];
  ...
  function blankState(type) {
    ...
    if (type === "edit") {
      ...
      if (!customer) {
        customer = { id: `cust-${Date.now()}`, name: inv.vendor, phone: "-", email: "", address: "Alamat vendor belum dilengkapi", bankName: "", accountHolder: "", accountNumber: "" };
    ...
    return `
      <div class="wizard-top card panel">
        <div class="section-head wizard-heading">
          <div>
            <div class="eyebrow">TRANSAKSI PEMBELIAN</div>
            <h1 class="panel-title">${state.mode === "edit" ? "Ubah" : "Tambah"} Pembelian</h1>
  ...
  function stepNote() {
    return `
      <div class="card panel wizard-card">
        ...
        <div class="source-lock"><div><strong>${esc(state.source)}</strong><span>Sumber data aktif dari halaman Daftar Pembelian</span></div><span class="lock-pill">🔒 Tidak dapat diubah</span></div>
        <div class="form-grid wizard-form-grid">
          ...
          <div class="field"><label>Referensi Invoice Vendor <span class="optional">Opsional</span></label><input value="${esc(state.vendorRef)}" placeholder="Contoh: INV/VDR/001" oninput="salesSetField('vendorRef', this.value)" /></div>
        ...
        <div class="wizard-footer"><span></span><button class="btn btn-primary" onclick="salesNextStep()">Lanjut ke Vendor →</button></div>
      </div>`;
  }
  ...
  function customerResults() {
    ...
    return results.length ? results.map(item => `
      ...
      </button>`).join("") : `<div class="empty-result">Vendor tidak ditemukan. Tambahkan sebagai vendor baru.</div>`;
  }
  ...
  function stepCustomer() {
    ...
    return `
      <div class="card panel wizard-card">
        <div class="wizard-section-title"><span>2</span><div><h2>Informasi Vendor</h2><p>Cari berdasarkan nama atau nomor telepon, atau tambahkan vendor baru.</p></div></div>
        <div class="customer-toolbar">
          ...
          <button class="btn btn-soft" onclick="salesOpenCustomerModal()">＋ Tambah Vendor</button>
        </div>
        <div class="customer-layout">
          ...
          <aside class="selected-customer-card ${customer ? "has-data" : ""}">
            ...
            ` : `<div class="customer-placeholder"><span>♙</span><strong>Belum ada vendor dipilih</strong><small>Pilih vendor dari daftar di sebelah kiri.</small></div>`}
          </aside>
        </div>
      ...
      <div class="wizard-columns">
        <div class="card panel wizard-card item-workspace">
          ...
          <div class="transaction-extras">
            <div class="field"><label>Catatan Transaksi</label><textarea placeholder="Catatan untuk vendor atau tim internal..." oninput="salesSetField('note', this.value)">${esc(state.note)}</textarea></div>
  ...
  function paymentFields(calc) {
    ...
    return `
      ...
      <div class="payment-choice compact-choice">
        <button class="${state.paymentMethod === "cash" ? "selected" : ""}" onclick="salesSetPaymentMethod('cash')"><span>💵</span><strong>Tunai</strong><small>Dibayarkan melalui kas</small></button>
        <button class="${state.paymentMethod === "transfer" ? "selected" : ""}" onclick="salesSetPaymentMethod('transfer')"><span>⇄</span><strong>Transfer</strong><small>Dibayarkan melalui bank</small></button>
      </div>
      <div class="form-grid wizard-form-grid payment-form">
        ...
        ${state.paymentMethod === "cash" ? `
          <div class="field"><label>Kas Pembayar <span class="required">*</span></label><select onchange="salesSetField('cashAccount', this.value)"><option>Kas Utama</option><option>Kas Toko Retail</option><option>Kas Roastery</option></select></div>
          <div class="field"><label>Uang Dibayarkan <span class="required">*</span></label>${rupiahInput(state.cashReceived, `oninput="salesMoneyInput('cashReceived', this); salesRefreshPaymentSummary()"`)}<button type="button" class="exact-money-btn" onclick="salesUseExactCash()">✓ Gunakan Uang Pas (${money(paymentAmount)})</button></div>
        ...
        ` : `
          <div class="payment-subsection full"><div><strong>Data Rekening Pembayar</strong><small>Rekening aktif milik ${esc(state.source)}</small></div></div>
          <div class="field"><label>Rekening Pengirim/Pembayar <span class="required">*</span></label><select onchange="salesSetReceiverAccount(this.value)">${receiverAccounts.map(account => `<option value="${account.id}" ${state.receiverAccount === account.id ? "selected" : ""}>${esc(account.label)} • ${esc(account.number)}</option>`).join("")}</select></div>
          <div class="field"><label>Nama Pemilik Rekening Pembayar</label><input class="disabled" value="${esc(receiver.owner)}" disabled /><div class="field-hint">Otomatis mengikuti Rekening Pembayar</div></div>
          <div class="payment-subsection full"><div><strong>Data Rekening Penerima</strong><small>Rekening milik vendor yang menerima pembayaran</small></div></div>
          <div class="field"><label>Nama Bank Penerima <span class="required">*</span></label><input value="${esc(state.senderBankName || customer?.bankName || "")}" oninput="salesSetField('senderBankName', this.value)" placeholder="Contoh: Bank BCA" /></div>
          <div class="field"><label>Nama Pemilik Rekening Penerima <span class="required">*</span></label><input value="${esc(state.senderAccountHolder || customer?.accountHolder || "")}" oninput="salesSetField('senderAccountHolder', this.value)" /></div>
          <div class="field"><label>Nomor Rekening Penerima <span class="required">*</span></label><input value="${esc(state.senderAccountNumber || customer?.accountNumber || "")}" oninput="salesSetField('senderAccountNumber', this.value)" /></div>
          <label class="check-option full sender-save-option"><input type="checkbox" ${state.saveSenderAccount ? "checked" : ""} onchange="salesSetField('saveSenderAccount', this.checked)" /> <span><strong>Simpan rekening ini ke data vendor</strong><small>Informasi rekening penerima dapat digunakan pada transaksi berikutnya.</small></span></label>
        `}
      ...
      <div class="wizard-columns payment-layout">
        <div class="card panel wizard-card">
          ...
          <div class="payment-choice">
            ...
            <button class="${state.paymentType === "later" ? "selected" : ""}" onclick="salesSetPaymentType('later')"><span>◷</span><strong>Bayar Nanti</strong><small>Terbitkan sebagai hutang</small></button>
          </div>
  ...
  function customerModal() {
    return `<div class="modal-backdrop" id="salesCustomerModal"><div class="modal customer-modal"><div class="modal-head"><div><div class="modal-title">Tambah Vendor</div><div class="sub-text">Vendor baru akan langsung dipilih pada transaksi.</div></div><button class="close" onclick="salesCloseCustomerModal()">×</button></div>
      <div class="form-grid wizard-form-grid">
        <div class="field"><label>Nama Vendor <span class="required">*</span></label><input id="newCustomerName" placeholder="Nama perusahaan atau vendor" /></div>
        ...
        <div class="field full"><label>Alamat <span class="required">*</span></label><textarea id="newCustomerAddress" placeholder="Alamat lengkap vendor"></textarea></div>
      </div>
  ...
  function validateStep(step) {
    ...
    if (step === 2 && !selectedCustomer()) return "Pilih vendor sebelum melanjutkan.";
    if (step === 3) {
    ...
    if (step === 4) {
      ...
      if (state.paymentType === "later") {
        ...
        if (state.paymentMethod === "cash") {
          if (!state.cashAccount) return "Pilih Kas Pembayar.";
          if ((Number(state.cashReceived) || 0) < paid) return "Uang dibayarkan tidak boleh lebih kecil dari nominal pembayaran.";
        } else {
          const customer = selectedCustomer();
          ...
          if (!state.receiverAccount || !bank || !holder || !account) return "Lengkapi rekening pembayar perusahaan dan rekening penerima vendor.";
        }
      }
  ...
  function buildPayload(status) {
    ...
    return {
      ...
      changes: oldInvoice
        ...
        : [status === "Draft" ? "Draft pembelian dibuat" : "Transaksi terbit dibuat oleh Siti Mulyani"]
    };
  }
  ...
  function persist(status, createAnother) {
    ...
    renderList();
    if (status === "Draft") {
      showToast("Draft pembelian berhasil disimpan.");
      showPage("detail");
      return;
    }
    if (createAnother) {
      showToast("Pembelian berhasil disimpan. Form baru telah disiapkan.");
      window.openForm("create");
    } else {
      showToast("Pembelian berhasil disimpan dan diterbitkan.");
      showPage("detail");
    }
  }
  ...
  window.salesExitWizard = function () {
    if (state.dirty && !window.confirm("Data yang telah diisi akan hilang. Keluar dari Tambah Pembelian?")) return;
    state.dirty = false;
    showPage("list");
  };
  ...
  window.salesSaveCustomer = function () {
    ...
    setDirty();
    showToast("Vendor baru berhasil ditambahkan dan dipilih.");
    renderWizard();
  };
